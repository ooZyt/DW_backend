...
public interface ReleaseTimeMapper {
    ...
    @Select("SELECT COUNT(*) FROM movie m JOIN release_time rt ON m.movie_id = rt.movie_id WHERE DAYOFWEEK(STR_TO_DATE(CONCAT(rt.year, '-', rt.month, '-', rt.day), '%Y-%c-%e')) = #{dayOfWeek}")
    int countMoviesByDayOfWeek(@Param("dayOfWeek") int dayOfWeek);
